...
def plot_file(file, ratio, tmax, Nt, origin, show_points=True):
    ...
    sx_inter, sy_inter = interp.gen_equation_mma(t_begin)
    t_end = t_begin + tmax * 2 * np.pi
    eq_x += '(' + sx_inter + f')*HeavisideTheta[t-{to_frac(t_begin)}]*HeavisideTheta[{to_frac(t_end)}-t]+'
    eq_y += '(' + sy_inter + f')*HeavisideTheta[t-{to_frac(t_begin)}]*HeavisideTheta[{to_frac(t_end)}-t]+'
    t_begin = t_end
...
fout_x = open('../Mathematica/diana_x', 'w')
fout_x.write('x[t_]:=')
fout_x.write(eq_x)
fout_x.write('\n')

fout_y = open('../Mathematica/diana_y', 'w')
fout_y.write('y[t_]:=')
fout_y.write(eq_y)
fout_y.write('\n')
