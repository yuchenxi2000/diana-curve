import re
import numpy as np
from fourier import FourierInterp, to_frac
...
def plot_file(file, ratio, tmax, Nt, origin, show_points=True):
    # 因为懒不改函数名了。。。
    global eq_x, eq_y, t_begin
    interp, points = get_fourier(file, ratio, origin)
    sx_inter, sy_inter = interp.gen_equation(t_begin)
    t_end = t_begin + tmax * 2 * np.pi
    eq_x += '(' + sx_inter + f').*(t>={to_frac(t_begin)}).*(t<{to_frac(t_end)})+'
    eq_y += '(' + sy_inter + f').*(t>={to_frac(t_begin)}).*(t<{to_frac(t_end)})+'
    t_begin = t_end
...
fout_x = open('../matlab/diana_x.m', 'w')
fout_x.write('function out = diana_x(t)\n    out = ')
fout_x.write(eq_x)
fout_x.write(';\nend\n')

fout_y = open('../matlab/diana_y.m', 'w')
fout_y.write('function out = diana_y(t)\n    out = ')
fout_y.write(eq_y)
fout_y.write(';\nend\n')

fout_main = open('../matlab/plot_diana.m', 'w')
fout_main.write(f's=0:0.01:{t_begin};\n')
fout_main.write("""
x = diana_x(s);
y = diana_y(s);
