import re
import numpy as np
from fourier import FourierInterp, to_frac_tex
...
def plot_file(file, ratio, tmax, Nt, origin, show_points=True):
    ...
    sx_inter, sy_inter = interp.gen_equation_tex(t_begin)
    t_end = t_begin + tmax * 2 * np.pi
    eq_x += '(' + sx_inter + f')\\Theta(t-{to_frac_tex(t_begin)})\\Theta({to_frac_tex(t_end)}-t)+'
    eq_y += '(' + sy_inter + f')\\Theta(t-{to_frac_tex(t_begin)})\\Theta({to_frac_tex(t_end)}-t)+'
    t_begin = t_end
...
fout_x = open('../tex/diana_x.tex', 'w')
fout_x.write('\\begin{equation}\nx=x(t)=\n')
fout_x.write(eq_x)
fout_x.write('\n\\end{equation}\n')

fout_y = open('../tex/diana_y.tex', 'w')
fout_y.write('\\begin{equation}\ny=y(t)=\n')
fout_y.write(eq_y)
fout_y.write('\n\\end{equation}\n')
